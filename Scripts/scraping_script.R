...
MI_D_text <- pdf_text(pdf_subset("data/MI_D.pdf", pages = 3:33)) %>% #unable to download directly from site
  str_replace_all("\\t|\\r|•"," ") %>% 
  tolower() %>%
  str_replace_all("[:punct:]"," ") %>%
  str_split("\n")
...
NV_D_text <- pdf_text('data/NV_D.pdf') #unable to download directly from site
...
OR_D_text <- pdf_text("data/OR_D.pdf") %>% #pdf housed on Google Drive, manually downloaded
  str_replace_all("\\t|\\r|•"," ") %>% 
  tolower() %>%
  str_replace_all("[:punct:]"," ") %>%
  str_split("\n")
...
TX_R_text <- pdf_text(pdf_subset("data/TX_R.pdf", pages = 2:33))
...
WI_D_text <- pdf_text("data/WI_D.pdf") %>% #blocked from scraping pdf
  str_replace_all("\\t|\\r|•"," ") %>% 
  tolower() %>%
  str_replace_all("[:punct:]"," ") %>%
  str_split("\n")
...
saveRDS(D_text_raw, "data/D_text_raw.rds")
...
saveRDS(R_text_raw, "data/R_text_raw.rds")
...
saveRDS(trump_text_raw, "data/trump_text_raw.rds")
  removeNumbers()